...
export default async function handler(
  ...
  const { assetTag, status, newUser, oldUser, remarks, department, position } =
    req.body;
  ...
  try {
    ...
    const spareAssets = await inventoryCollection
      .find({ status: "spare" })
      .toArray();
    const deployedAssets = await inventoryCollection
      .find({ status: "deployed" })
      .toArray();
