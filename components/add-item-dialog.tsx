...
export function AddItemDialog({
  ...
  // Initialize formData once from stable fields (fields is static per your input)
  const initialFormData = React.useMemo(() => {
    ...
    allFields.forEach((field) => {
      initial[field.key] = (field as any).defaultValue ?? "";
    });
    initial["remarks"] = "";
    return initial;
  }, [fields, pageType]);
  ...
  React.useEffect(() => {
    if (!open) {
      ...
      // Reset formData when dialog closed to initial values (optional)
  ...
  }, [open, pageType]); // initialFormData intentionally not added to avoid unwanted resets while typing
  ...
  const getStatusOptions = React.useCallback(() => {
    ...
    return pageType
      ? statusByPage[pageType] || []
      : ["deployed", "spare", "lend", "defective", "dispose", "missing"];
  }, [pageType]);
  ...
  const handleAssetTypeChange = React.useCallback(async (value: string) => {
  ...
  const computeAssetAge = React.useCallback((date: Date) => {
    const now = new Date();
  ...
  const handleAssetSelect = React.useCallback((asset: any) => {
    setFormData((prev) => ({
      ...prev,
    ...
    try {
      ...
      if (formData.purchaseDate) {
        ...
        const isOver5Years =
          diffYears > 5 ||
          (diffYears === 5 &&
            (monthCheck > 0 || (monthCheck === 0 && dayCheck >= 0)));
        if (isOver5Years) {
          autoStatus = "dispose";
          toast.info(
            "Asset is over 5 years old. Status automatically set to 'dispose'."
          );
        }
      }
      ...
      if (pageType === "assigned" && formData.assetTag) {
        try {
          const statusRes = await fetch(
            "/api/backend/inventory/update-status",
            {
              method: "POST",
              headers: { "Content-Type": "application/json" },
              body: JSON.stringify({
          ...
          if (!statusRes.ok) {
            toast.warning(
              "Assignment created but failed to update asset status. Please update manually."
            );
          }
        } catch (statusError) {
          toast.warning(
  ...
  }, [fields, fetchPosts, formData, onOpenChange, pageType, userDetails]);
  ...
  // stableFields is memoized and returns same object reference unless pageType or fields change
  const stableFields = React.useMemo(() => {
    return pageType === "assigned"
      ? [
          {
            key: "assetTag",
            label: "Asset Tag",
  ...
  // Asset selection UI (memoized JSX)
  const assetSelectionJSX = React.useMemo(() => {
    return (
            ...
            <div className="grid grid-cols-1 gap-3 mb-4">
              {spareAssets
                ...
                .map((asset) => {
                  ...
                  const storageValue = asset.storage
                    ? asset.storage.toLowerCase()
                    : "";
                  let storageGB = 0;
                  if (storageValue.includes("tb"))
                    storageGB = parseFloat(storageValue) * 1024;
                  else if (storageValue.includes("gb"))
                    storageGB = parseFloat(storageValue);
                      ...
                      <Card>
                        ...
                        <CardContent className="pl-6">
                          <p className="text-xs text-muted-foreground mb-2">
                            This asset is a {asset.brand} {asset.model} with{" "}
                            {asset.ram} RAM, powered by a {asset.processor}{" "}
                            processor and {asset.storage} storage. Perfect for
                            office or technical use.
                          </p>
    ...
    // selectedAsset is intentionally not added to deps to avoid remount on every keystroke,
    // selectedAsset changes will still reflect because setSelectedAsset triggers rerender.
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [isLoadingAssets, spareAssets, onOpenChange, handleAssetSelect]);

  // Form UI (memoized JSX). depends on stableFields, formData, purchaseDate, isCalendarOpen, options
  const formJSX = React.useMemo(() => {
    return (
      <div className="max-h-[70vh] overflow-y-auto px-1 space-y-4 [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none]">
                ...
                <div key={field.key} className="flex flex-col">
                  ...
                  {isSelect ? (
                    ...
                    <Input
                      ...
                      className={
                        isDisabled || (isAssetTag && pageType !== "assigned")
        ...
        <div className="mt-4">
          <Label htmlFor="remarks" className="text-sm font-medium text-muted-foreground mb-1">
            Remarks
          </Label>
          <Textarea
            id="remarks"
  ...
  const DialogBody = step === "selectAsset" && pageType === "assigned" ? assetSelectionJSX : formJSX;
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      ...
      <DialogContent className={`${sizeClasses[dialogSize]} p-6`}>
        <DialogHeader>
          <DialogTitle>
            {step === "selectAsset" && pageType === "assigned" ? "Select Spare Asset" : "Add New Item"}
          </DialogTitle>
          <DialogDescription>
