  ...
  export function AddItemDialog({
    ...
    const handleAddNew = () => {
      ...
      const newItem: Record<string, any> = {
        id: Math.max(...filteredData.map((item) => item.id), 0) + 1,
        ...formData,
      }
    ...
    const DialogBody = (
      <div className="max-h-[70vh] overflow-y-auto px-1 space-y-4 [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none]">
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-6 gap-y-3">
                ...
                <div key={field.key} className="flex flex-col">
                  ...
                  {isSelect ? (
                    ...
                    <Input
                      ...
                      readOnly={isAssetTag}
                      className={isAssetTag ? "bg-gray-50 text-gray-600 cursor-not-allowed" : ""}
                    />
                  )}
                </div>
              )
          ...
          {/* --- Asset Age (Right) --- */}
          <div className="flex flex-col">
            <Label htmlFor="assetAge" className="text-sm font-medium text-muted-foreground mb-1">
              Asset Age
            </Label>
            <Input
              id="assetAge"
      ...
      <Dialog open={open} onOpenChange={onOpenChange}>
        ...
        <DialogContent className={`${sizeClasses[dialogSize]} p-6`}>
          <DialogHeader>
            ...
            <DialogDescription>
              Fill in the details to add a new item to the table.
            </DialogDescription>
          </DialogHeader>
          {DialogBody}
          {DialogFooterButtons}
